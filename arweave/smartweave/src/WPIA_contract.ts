...
function transfer_WPIA(state: State, caller: string, target: string, qty: number) {
    /*
...
function transfer_SOL_WPIA(state: State, target: string, qty: number) {
    /*
    ** Incomming transcation from validated waiting transaction.
    **
    ** target: wallet id of the receving account
    ** qty: Winston to transfer
    */
...
function transfer_WPIA_SOL(state: State, caller: string, target: string, qty: number) {
    /*
    ...
    */
    ...
    if (caller in balances) {
        if (balances[caller] > qty) {
            ...
            waiting_txs[Smartweave.transaction.id] = {"owner": caller, "target": target, "target_ticker": "SOL", "ticker": "WPIA", "qty": qty, "vote": {} as Vote};
        } else {
            throw new (ContractError as any)(`No enough balance from '${caller}'.`);
        }
    } else {
    ...
    */
    ...
    if (waiting_txs_vote.voter.length >= waiting_txs_vote.min_vote) {
        ...
        if (validator === "error" || waiting_txs_vote.voter[validator] === 0) {
            ...
            transfer_SOL_WPIA(state, waiting_txs.target, waiting_txs.qty);
...
export async function handle(state: State, action: Action): Promise<{state?: State, result?:unknown}> {
    ...
    if (input.function === 'transfer') {
        ...
        if (ticker === "WPIA" && ticker_target === "WPIA") {
            transfer_WPIA(state, caller, target, qty);
        } else if (ticker === "WPIA" && ticker_target === "SOL") {
            transfer_WPIA_SOL(state, caller, target, qty);
        } else {
            throw new (ContractError as any)(`No transfer allowed between: 'WPIA' and '${ticker_target}'.`);
        }
        return { state };
    }
